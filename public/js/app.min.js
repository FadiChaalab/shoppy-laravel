!(function(t) {
	'use strict';
	function s(e) {
		console.log('updateThemeSetting', e),

				1 == t('#light-mode-switch').prop('checked') && 'light-mode-switch' === e ? (t(
					'#dark-mode-switch'
				).prop('checked', !1),
				t('#rtl-mode-switch').prop('checked', !1),
				t('#bootstrap-style').attr('href', 'css/bootstrap.min.css'),
				t('#app-style').attr('href', 'css/app.min.css'),
				sessionStorage.setItem('is_visited', 'light-mode-switch')) :
				1 == t('#dark-mode-switch').prop('checked') && 'dark-mode-switch' === e ? (t('#light-mode-switch').prop(
					'checked',
					!1
				),
				t('#rtl-mode-switch').prop('checked', !1),
				t('#bootstrap-style').attr('href', 'css/bootstrap-dark.min.css'),
				t('#app-style').attr('href', 'css/app-dark.min.css'),
		...
		t('#vertical-menu-btn').on('click', function(e) {
			e.preventDefault(),
				t('body').toggleClass('sidebar-enable'),
					...
					992 <= t(window).width() ? t('body').toggleClass('vertical-collpsed') :
		...
		(function() {
			if (window.sessionStorage) {
				var e = sessionStorage.getItem('is_visited');
					...
					e ? (t('.right-bar input:checkbox').prop('checked', !1), t('#' + e).prop('checked', !0), s(e)) :
					sessionStorage.setItem('is_visited', 'light-mode-switch');
			}
